    ...
    <>
      ...
      <div className="min-h-screen overflow-hidden bg-black text-foreground">
            ...
            <div className="grid items-center gap-16 lg:grid-cols-[1.05fr_0.95fr]">
              <div className="max-w-3xl">
                ...
                >
                  <a
                    href="#system"
                    className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-8 py-4 text-sm font-semibold text-black transition hover:bg-primary/90"
                  >
                    Descobrir o sistema
                  ...
                  <a
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 px-8 py-4 text-sm font-semibold text-white transition hover:border-primary hover:text-primary"
                  >
                    Ir para a loja
                  ...
                  <div className="space-y-3">
                    {steps.map((step, index) => {
                      const systemRow = (
                        <>
                          <span
                            className={`text-xs font-semibold ${step.accent}`}
                          >
                            {step.number}
                          </span>
                          ...
                          {step.href ? (
                            <ArrowRight
                              className={`h-4 w-4 ${step.accent} opacity-60 transition-all duration-300 group-hover:translate-x-1 group-hover:opacity-100`}
                            />
                          ) : (
                        ...
                        >
                          {step.href ? (
                            <a
                              ...
                              className={`group flex items-center gap-5 rounded-2xl border ${step.border} bg-gradient-to-r ${step.glow} to-transparent px-5 py-4 transition duration-300 hover:-translate-y-0.5 hover:bg-white/[0.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70`}
                            >
                              {systemRow}
                            </a>
                          ) : (
                            <div
                              className={`group flex items-center gap-5 rounded-2xl border ${step.border} bg-gradient-to-r ${step.glow} to-transparent px-5 py-4`}
                            >
                              {systemRow}
                            </div>
                          )}
        ...
        <section className="border-b border-white/5 bg-black py-16 md:py-20">
          ...
          >
            <p className="mb-5 w-full text-center text-xs font-semibold uppercase tracking-[0.4em] text-primary">
              O ponto de partida
            </p>
            ...
            >
              ...
              <span className="block text-white/55">
                o couro cabeludo passa a viver exposto.
              </span>
            </h2>

            <p className="mx-auto mt-7 max-w-3xl text-base leading-relaxed text-white/60 md:text-lg">
              Sol, calor, suor, oleosidade e ressecamento entram na rotina.
              Quem é careca conhece essa realidade. A BaldShield organiza
              esse cuidado em um sistema simples, específico e contínuo.
            </p>
          </motion.div>
        </section>
        ...
        <section
          id="system"
          className="scroll-mt-24 bg-gradient-to-b from-black via-zinc-950 to-black py-24 md:py-28"
        >
          <div className="mx-auto max-w-7xl px-6 sm:px-8 lg:px-10">
            ...
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
              {steps.map((step, index) => {
                const cardContent = (
                  <>
                    <div className="absolute right-5 top-2 text-[5rem] font-bold leading-none text-white/[0.035]">
                      {step.number}
                    </div>
                    ...
                    <div className="relative flex h-full flex-col">
                      <div className="mb-10 flex items-center justify-between">
                        <span
                          className={`text-xs font-semibold tracking-[0.25em] ${step.accent}`}
                        >
                          {step.number}
                        </span>
                      ...
                      <p
                        className={`mb-3 flex min-h-[2.5rem] items-center text-xs font-semibold uppercase tracking-[0.22em] ${step.accent}`}
                      >
                        {step.role}
                      </p>
                      ...
                      {step.href ? (
                        <div
                          className={`mt-auto flex items-center gap-2 pt-8 text-xs font-semibold uppercase tracking-[0.16em] ${step.accent}`}
                        >
                          Conhecer {step.name}
                          <ArrowRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" />
                        </div>
                      ) : (
                        <div className="mt-auto pt-8 text-xs font-semibold uppercase tracking-[0.16em] text-white/25">
                          Em breve
                        </div>
                      )}
                    </div>
                ...
                const cardClasses = `group relative h-full overflow-hidden rounded-[2rem] border ${step.border} bg-gradient-to-b ${step.glow} via-black/70 to-black p-7 transition duration-300 ${
                  step.href
                    ? 'cursor-pointer hover:-translate-y-1 hover:border-opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70'
                    : ''
                }`;
            ...
            <div className="grid gap-12 lg:grid-cols-[0.75fr_1.25fr] lg:items-center">
              ...
              >
                ...
                <p className="mt-6 max-w-lg text-base leading-relaxed text-white/55">
                  Cada etapa tem uma função. Juntas, elas transformam
                  improviso em uma rotina organizada de cuidado.
                </p>
              </motion.div>
              ...
              >
                <div className="border-b border-white/10 pb-7">
                  <p className="mb-5 text-[10px] font-semibold uppercase tracking-[0.35em] text-white/35">
                    Comece o dia
                  </p>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    {steps.slice(0, 3).map((step, index) => (
                      <React.Fragment key={step.name}>
                        ...
                        {index < 2 && (
                          <ArrowRight className="hidden h-4 w-4 flex-shrink-0 text-primary/50 sm:block" />
                        )}
                      </React.Fragment>
                    ))}
                  </div>
